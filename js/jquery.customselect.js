...
(function($) {
	$.fn.customSelect = function(settings) {
		var config = {
			...
			activeClass: 'select_active', // Class name assigned to the fake select when the real select is in hover/focus state
		...
		this.each(function() {
			var selected = 'select_selected';
			var select = $(this);

			if (select.is(":disabled")) {
				select.addClass('select_disabled');
			}
			...
			var update = function() {
				...
				if (!selectWrap.hasClass('select_no-selected')) {
			...
			var span = $('<span class="' + config.customSelectClass + '" aria-hidden="true"><span class="select__bg"><span>' + $('option:selected', this).text() + '</span></span><span class="select__arrow"><svg class="select__icon"><use xlink:href="#icon_down-arrow"></use></svg></span></span>');
			select.after(span);
